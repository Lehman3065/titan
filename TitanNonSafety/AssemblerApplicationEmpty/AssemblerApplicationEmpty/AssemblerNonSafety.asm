...
.EQU	Duration_0 = (Fclk/4)	; длительность интервала при передаче 0 (0,25 мс)
.EQU	Duration_1 = (Fclk/2)	; длительность интервала при передаче 1 (0,5 мс) - передача 30 бит гарантировано укладывается в период
...
/*	init PORTA	*/
	...
	WriteByteToReg_r16 DDRA, (0xFF - (1<<bLedIn) - (1<<bLedT1))	; PA4, PA7 - input
...
SendBuffer:					; - must be MACRO ???
	...
	ldi r18, (LOW(SendStart))	; стартовый импульс
	ldi r19, (HIGH(SendStart))
	rcall SendImpuls
	ldi r18, (LOW(DurationStart))	; стартовый интервал
	ldi r19, (HIGH(DurationStart))
	rcall SendImpuls
...
SendCarry:
	brcs SendCarry1
	ldi r18, (LOW(Duration_0))
	ldi r19, (HIGH(Duration_0))
	rjmp SendImpuls
SendCarry1:
	ldi r18, (LOW(Duration_1))
	ldi r19, (HIGH(Duration_1))
	rjmp SendImpuls

SendImpuls:
	add r20, r18
	adc r21, r19
	out OCR1BH, r21		; устанавливаем момент включения оптрона
	out OCR1BL, r20
	WriteByteToReg_r16 TCCR1A, ((1<<COM1B1)|(1<<COM1B0))	; Set OC1B on Compare Match
	WriteByteToReg_r16  TIMSK, (1<<OCIE1B); разрешить прерывание TIM1_COMPB_ISR

Wait_Impuls:
;	rcall Receive_r16	; прием и, если приема нет - инкремент счетчика неприема на 0 или 1
	in r16, TCCR1A
	sbrc r16, COM1B1	; только COM1B1, так как сюда ещё и rjmp из WaitFrameEnd
	rjmp Wait_Impuls
	ret
...
/* 
WaitFrameEnd: 
	Load16 r20, r21, MachLength	; MachLength
	Load16 r18, r19, TStartPeriod	; начальное значение периода 0
	add r20, r18
	adc r21, r19				; предполагаемый конец кадра
	subi r20, (LOW(EndOfFrame))	; заранее - за EndOfFrame
	sbci r21, (HIGH(EndOfFrame))

	out OCR1BH, r21				; устанавливаем момент перехода к ожиданию конца кадра
	out OCR1BL, r20

	WriteByteToReg_r16 TCCR1A, (1<<COM1B1)	; Clear OC1B on Compare Match
	WriteByteToReg_r16  TIMSK, (1<<OCIE1B)	; разрешить прерывание TIM1_COMPB_ISR
	rjmp Wait_Impuls
*/
...
WaitPhaseFalse:
	...
	WaitFalse PINB, bPhase	; ждать (-) полуволну фазы

	rcall SetTimeSamples	;  - must be MACRO ????
...
/*
SetTSamples:
	Load16 r24, r25, T1Value	; Вычитаем CorrectionT, так как фиксация начала цикла отстает от перехода фазы через ноль
	ldi r22, (LOW(CorrectionT))	; и плюс ещё и запуск ADC отстает от срабатывания таймера Т1 - и там и там добавляется время
	ldi r23, (HIGH(CorrectionT)); обработки прерывания
	sub16u r24, r25, r22, r23	; r24:r25 = TStartPeriod
	Store16 TStartPeriod, r24, r25	; Старт 0-го периода (счет идет с нулевого!)
	
	Load16 r24, r25, T1Value	; Вычисляем длину кадра MachLength
	Load16 r22, r23, PreT1Value
	sub16u r24, r25, r22, r23	; MachLength = r24:r25 = r24:r25 - r22:r23
	Store16 MachLength, r24, r25; MachLength = T1Value - PreT1Value

	div16u2 r24, r25			; MachLength/2
	Load16 r22, r23, TStartPeriod
	add16u r22, r23, r24, r25
	Store16 StartT4, r22, r23 ; Старт 4-го периода (счет идет с нулевого!)

	div16u2 r24, r25			; MachLength/4
	Load16 r22, r23, TStartPeriod
	add16u r22, r23, r24, r25
	Store16 StartT2, r22, r23 ; Старт 2-го периода (счет идет с нулевого!)

	div16u2 r24, r25			; MachLength/8
	add16u r22, r23, r24, r25
	Store16 StartT3, r22, r23	; Старт 3-го периода (счет идет с нулевого!)

	Load16 r22, r23, TStartPeriod
	add16u r22, r23, r24, r25
	Store16 StartT1, r22, r23	; Старт 1-го периода (счет идет с нулевого!)

	div16u2 r24, r25
	mov r22, r24
	mov r23, r25				; MachLength/16
	div16u2 r24, r25			; MachLength/32
	Store16 LengthDiv32, r24, r25

	add16u r22, r23, r24, r25
	Store16 StartTMaxP, r22, r23	; 3/4 периода сети

	div16u2 r24, r25				; MachLength/64
	Store16 LengthDiv64, r24, r25	; - четвертушка полупериода
	ret
*/

SetTimeSamples:
	lds r25, (T1Value + 1)
	lds r16, (PreT1Value + 1)
	sub r25, r16
	...
	CopyWordWord_r16 PreT1Value, T1Value ; сохраняем предыдущее значение Т1
